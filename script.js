...
function speakText(text) {
    ...
    setTimeout(() => {
        ...
        utterance.rate = 0.82; // Slower, calmer pace for children
...
async function handleLocationClick(feature, sourceView, layer = null) {
    ...
    try {
        ...
        if (countryCache[isoCode]) {
            ...
            // Find country in local data (allCountriesData) by cca3 code
            countryData = allCountriesData.find(c => c.cca3 === isoCode || c.cca2 === isoCode);
            if (!countryData) {
        ...
        if (koreanName === '조선' || koreanName === '조선민주주의인민공화국') {
            koreanName = '북한';
        } else if (koreanName === '한국' || koreanName === '남한') {
            koreanName = '대한민국';
        }
        const flagUrl = `flags/${countryData.cca2.toLowerCase()}.svg`;

        let capitalNameEng = countryData.capital && countryData.capital.length > 0 ? countryData.capital[0] : null;
        let capitalNameKor = '수도 정보 없음';
        if (capitalNameEng) {
            capitalNameKor = krCapitals[capitalNameEng] || capitalNameEng;
        }

        // mledoze/countries doesn't have capital specific coords, so we use the country center coords for the camera and marker
        const capitalLatLng = countryData.latlng;
        ...
        if (capitalNameKor === '수도 정보 없음' || capitalNameKor === '-') {
            speakText(`여기는 ${koreanName}입니다. 수도 정보가 없습니다.`);
        } else {
            speakText(`여기는 ${koreanName}입니다. 수도는 ${capitalNameKor}입니다.`);
        }
